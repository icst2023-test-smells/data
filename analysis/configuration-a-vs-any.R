...
cat('\\toprule\n', sep='')
cat('Project & \\# Classes & Vanilla & \\textit{other} & \\# Sig. Better & \\# Sig. Worse & \\# Sig. No diff. & \\# Better & \\# Worse & \\# No diff. \\\\\n', sep='')
cat('\\midrule\n', sep='')
...
data <- data.frame(group_id=character(), TARGET_CLASS=character(),
  ...
  num_sig_better=numeric(), num_sig_worse=numeric(), num_sig_no_diff=numeric(),
...
for (project in sort(unique(df$'group_id'))) {
  ...
  for (clazz in unique(df$'TARGET_CLASS'[project_mask])) {
    ...
    data[nrow(data)+1, ] <- c(
      ...
      num_sig_better=ifelse(a12 > 0.50 && p.value < 0.05, 1, 0),
      num_sig_worse=ifelse(a12 < 0.50 && p.value < 0.05, 1, 0),
      num_sig_no_diff=ifelse(a12 == 0.50 && p.value < 0.05, 1, 0),
  ...
  data$'num_sig_better'             <- as.numeric(data$'num_sig_better')
  data$'num_sig_worse'              <- as.numeric(data$'num_sig_worse')
  data$'num_sig_no_diff'            <- as.numeric(data$'num_sig_no_diff')
  ...
  cat(' & ', sum(data$'num_sig_better'[data$'group_id' == project]),
      ' & ', sum(data$'num_sig_worse'[data$'group_id' == project]),
      ' & ', sum(data$'num_sig_no_diff'[data$'group_id' == project]),
      sep='')
...
cat(' & ', sum(data$'num_sig_better'),  ' (', sprintf('%.2f', round(sum(data$'num_sig_better')  / length(unique(df$'TARGET_CLASS')) * 100.0, 2)), '\\%)', sep='')
cat(' & ', sum(data$'num_sig_worse'),   ' (', sprintf('%.2f', round(sum(data$'num_sig_worse')   / length(unique(df$'TARGET_CLASS')) * 100.0, 2)), '\\%)', sep='')
cat(' & ', sum(data$'num_sig_no_diff'), ' (', sprintf('%.2f', round(sum(data$'num_sig_no_diff') / length(unique(df$'TARGET_CLASS')) * 100.0, 2)), '\\%)', sep='')
...
print(summary(
  data[data$'num_sig_better' == 1, ]
))
print(relative_improvement_value(data$'a_value'[data$'num_sig_better' == 1], data$'b_value'[data$'num_sig_better' == 1])*100.0,)
print(summary(
...
print(summary(
  data[data$'num_sig_worse' == 1, ]
))
print(relative_improvement_value(data$'a_value'[data$'num_sig_worse' == 1], data$'b_value'[data$'num_sig_worse' == 1])*100.0,)
print(summary(
...
print(summary(
  data[data$'num_sig_better' == 1 | data$'num_sig_no_diff' == 1, ]
))
print(relative_improvement_value(data$'a_value'[data$'num_sig_better' == 1 | data$'num_sig_no_diff' == 1], data$'b_value'[data$'num_sig_better' == 1 | data$'num_sig_no_diff' == 1])*100.0,)
print(summary(
...
# Cases to be further discussed
data$'diff_a_vs_b' <- data$'a_value' - data$'b_value'
print('Sig better -- best cases')
print(data[data$'num_sig_better' == 1 & data$'diff_a_vs_b' == max(data$'diff_a_vs_b'[data$'num_sig_better' == 1]), ])
print('Sig worst -- worst cases')
print(data[data$'num_sig_worse'  == 1 & data$'diff_a_vs_b' == min(data$'diff_a_vs_b'[data$'num_sig_worse' == 1]), ])
