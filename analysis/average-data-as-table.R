...
df <- aggregate(as.formula(paste0('cbind(Size, Length, Generations, OverallCoverage, MutationScore, Smelliness, ', paste0(smells, collapse=','), ') ~ configuration_id + group_id + TARGET_CLASS + Random_Seed')), data=df, FUN=mean)
...
df <- aggregate(as.formula(paste0('cbind(Size, Length, Generations, OverallCoverage, RelativeOverallCoverage, MutationScore, RelativeMutationScore, Smelliness, RelativeSmelliness, ', paste0(smells, collapse=','), ', ', paste0(relative_smells, collapse=','), ') ~ configuration_id + group_id + TARGET_CLASS')), data=df, FUN=mean)
...
print_body <- function(df, cov, mut, smells, smellines, label) {
  ...
  for (configuration_id in unique(df$'configuration_id')) {
    ...
    cat(' (', mean(df$'Generations'[mask]), ')', sep='')
