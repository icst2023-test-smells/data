...
load_data <- function(zip_path, smells=c()) {
  ...
  # FIXME remove all rows for which there is no TestSmellVerboseTest value
  #
  # For some reason no smell was ever computed for the following two classes:
  # com.mytdev.resources4j.impl.AbstractResources
  # ch.bluepenguin.email.client.tapestry.MessageList
  #
  df <- df[! is.na(df$'TestSmellVerboseTest'), ]
...
compute_smelliness <- function(df, smells=c(), column_name='Smelliness') {
  ...
  df[[column_name]] <- rowMeans(df[ , smells], na.rm=TRUE)
